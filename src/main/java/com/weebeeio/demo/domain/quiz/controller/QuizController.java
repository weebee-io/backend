...
public class QuizController {
    ...
    QuizController(QuizService quizService, QuizResultService quizResultService, StatsService statsService, UserRepository userRepository) {
    ...
    public String grading(@PathVariable Integer user_id, @PathVariable Integer quiz_id, @PathVariable String answer) {
        ...
        if(quizResultService.findResultbyIdandQuizid(user_id,quiz_id).isEmpty()){
            QuizResultDao newQuizResult = new QuizResultDao();
            newQuizResult.setQuizId(quiz.get());
            Optional<User> user = userRepository.findById(user_id);
            if (user.isPresent()) {
                newQuizResult.setUser(user.get());
                newQuizResult.setIsCorrect(false);
                quizResultService.save(Optional.of(newQuizResult));
            }
            return "퀴즈 내역을 생성하였습니다.";
        }
        else{
