...
public class UserController {
    ...
    public ResponseEntity<CommonResponseDto<User>> getUserInfo(@PathVariable Integer userId) {
        User user = userService.getUserInfo(userId);
        return ResponseEntity.ok(CommonResponseDto.success(user, "회원 정보 조회가 완료되었습니다."));
    }
    ...
    public ResponseEntity<CommonResponseDto<User>> updateUserInfo(
        ...
        return ResponseEntity.ok(CommonResponseDto.success(user, "회원 정보 수정이 완료되었습니다."));
    }

    @DeleteMapping("/{userId}")
    @Operation(summary = "회원 탈퇴", description = "회원 탈퇴를 처리합니다.")
    public ResponseEntity<CommonResponseDto<Void>> deleteUser(@PathVariable Integer userId) {
        userService.deleteUser(userId);
        return ResponseEntity.ok(CommonResponseDto.success(null, "회원 탈퇴가 완료되었습니다."));
    }

    @GetMapping("/check-id/{id}")
    @Operation(summary = "아이디 중복 체크", description = "아이디 중복 여부를 확인합니다.")
    public ResponseEntity<CommonResponseDto<Boolean>> checkIdDuplicate(@PathVariable String id) {
        boolean isDuplicate = userService.checkIdDuplicate(id);
        return ResponseEntity.ok(CommonResponseDto.success(isDuplicate, 
